...
export function loadServiceAccount(path) {
    ...
    try {
        ...
        throw new Error(`google-vertex: cannot read service account file ${resolved}`
 ...
 */
export function signedAssertion(account, nowSeconds, scope) {
    const header = {
        alg: 'RS256',
        typ: 'JWT',
...
export class ServiceAccountTokens {
    ...
    async get(signal) {
        ...
        if (cached !== undefined && this.#now() < cached.expiresAt - DEFAULT_REFRESH_MARGIN_MS)
            return cached.value;
    ...
    async #mint(signal) {
        ...
        try {
            ...
            throw new VertexAuthError('TRANSPORT', `google-vertex: token endpoint unreachable — ${error instanceof Error ? error.message : String(error)}`);
        }
        const text = await response.text().catch(() => '');
        if (!response.ok) {
            throw new VertexAuthError('AUTH', `google-vertex: token endpoint refused the service account (HTTP ${response.status})`
